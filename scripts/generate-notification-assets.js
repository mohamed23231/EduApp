...
function generateBellIcon() {
  ...
  function fillCircle(cx, cy, r) {
    for (let y = Math.ceil(cy - r - 1); y <= cy + r + 1; y++) {
      for (let x = Math.ceil(cx - r - 1); x <= cx + r + 1; x++) {
        ...
        if (d < r - 0.5)      setPixel(x, y, 1.0);
        else if (d < r + 0.5) setPixel(x, y, r + 0.5 - d);
      }
    }
  }
  ...
  function fillTrap(yTop, yBot, halfWTop, halfWBot, cx) {
    for (let y = yTop; y <= yBot; y++) {
      ...
      for (let x = Math.floor(cx - halfW - 1); x <= cx + halfW + 1; x++) {
        ...
        if (dx < halfW - 0.5)      setPixel(x, y, 1.0);
        else if (dx < halfW + 0.5) setPixel(x, y, halfW + 0.5 - dx);
      }
    }
  }
  ...
  for (let y = 20; y <= 45; y++) {
    for (let x = 14; x <= 82; x++) {
      ...
      if (d < 24 - 0.5)      setPixel(x, y, 1.0);
      else if (d < 24 + 0.5) setPixel(x, y, 24 + 0.5 - d);
    }
  }
  ...
  for (let y = 68; y <= 78; y++) {
    for (let x = 10; x <= 86; x++) {
      ...
      if (dx > 28 && dy >= 0) {
        ...
        if (d < 38 - 0.5)       setPixel(x, y, 1.0);
        else if (d < 38 + 0.5)  setPixel(x, y, 38 + 0.5 - d);
      }
    }
  }
