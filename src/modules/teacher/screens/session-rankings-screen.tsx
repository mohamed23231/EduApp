...
import { ActivityIndicator, FlatList, I18nManager, Pressable, ScrollView, StyleSheet, View } from 'react-native';
...
type InsufficientItem = { studentId: string; studentName: string; ratedSessionsCount: number };
  ...
  return (
    <View>
      ...
      {items.map(item => (
        <Pressable
          key={item.studentId}
          onPress={() => onStudentPress(item.studentId)}
...
export function SessionRankingsScreen() {
  ...
  const renderRankedItem = ({ item }: { item: NonNullable<typeof data>['rankings'][number] }) => {
    ...
    return (
      <Pressable
        onPress={() => navigateToStudent(item.studentId)}
        style={({ pressed }) => [styles.rankRow, pressed && styles.rankRowPressed]}
        accessibilityRole="button"
      >
    ...
    <SafeAreaView edges={['top']} style={styles.container}>
      ...
      {!isLoading && !isError && data && data.rankings.length > 0 && (
        <FlatList
          ...
          ListFooterComponent={
            <InsufficientDataSection items={data.insufficientData} onStudentPress={navigateToStudent} />
          }
...
const styles = StyleSheet.create({
  ...
  rankRowPressed: { opacity: 0.7, backgroundColor: '#F9FAFB' },
